...
def authorization():
    ...
    return auth, api, client, stop_words

def getWordData(word):
    sid = SentimentIntensityAnalyzer()

    auth, api, client, stop_words = authorization()

    search_term = word
    search_term_set = set(search_term.lower().split())
    search_query = search_term + " lang:en -is:verified -is:retweet" # Only unverified accounts used to get real views
